...
@pytest.mark.asyncio
async def test_recommend_guard_agent(load_case):
    """Run all recommend guard regression cases."""
    case_list = load_case("regression/agent/agent_recommend_guard.json")
    cases = case_list if isinstance(case_list, list) else [case_list]

    runner = EvalRunner(case_dirs=["regression"])

    for case in cases:
        if case.get("agent") != "recommend_guard":
            continue
