...
class TicketExecuteRuntimePayload(BaseModel):
    ...
    slots: Dict[str, Any] = Field(default_factory=dict)
    expected_slots: List[Dict[str, str]] = Field(default_factory=list)


class FinalReplyContext(BaseModel):
    service_key: str = ""
    goal: str = ""
    final_status: str = ""
    final_reason: str = ""
...
def build_ticket_execute_runtime_context(
    ...
    rendered = format_context_sections(
        {
            "已有槽位": _serialize_runtime_value(payload.slots),
            "预期槽位定义": _serialize_runtime_value(payload.expected_slots),
            "当前步骤详情": _serialize_runtime_value(payload.step),
        }
    )
    return rendered or "[None]"
...
async def build_router_system_prompt(
    *,
    ...
    template = load_prompt("router/router.txt")
    return with_prompt_prefix(
        template.format(capability_context=rendered_capability_context)
    )
...
async def build_qa_system_prompt(
    *,
    ...
    prompt_user_ctx = _build_prompt_user_context(user_context=user_context)
    rendered_user_context = format_user_context(prompt_user_ctx)
...
async def build_recommend_guard_system_prompt() -> str:
    return build_base_system_prompt(prompt_file="recommend/guard.txt")
...
async def build_ticket_final_reply_prompt(
    *,
    ...
    rendered = format_context_sections(
        {
            ...
            "用户画像摘要": str(payload.profile_summary or "").strip(),
        }
    )
    return rendered or "[None]"
