...
class GetUserOrdersInput(BaseModel):
    ...
    order_start_time: Optional[str] = Field(default=None, description="订单创建开始时间，ISO8601。")
    order_end_time: Optional[str] = Field(default=None, description="订单创建结束时间，ISO8601。")
...
class GetUserScoreInput(BaseModel):
    page: int = Field(default=1, ge=1, description="页码，默认 1。")
    page_size: int = Field(default=20, ge=1, le=100, description="每页数量，默认 20，最大 100。")
    score_start_time: Optional[str] = Field(default=None, description="积分变动开始时间，ISO8601。")
    score_end_time: Optional[str] = Field(default=None, description="积分变动结束时间，ISO8601。")
...
class GetTicketsInput(BaseModel):
    ...
    ticket_start_time: Optional[str] = Field(default=None, description="工单创建开始时间，ISO8601。")
    ticket_end_time: Optional[str] = Field(default=None, description="工单创建结束时间，ISO8601。")
    page: int = Field(default=1, ge=1, description="页码，默认 1。")
    page_size: int = Field(default=20, ge=1, le=100, description="每页数量，默认 20，最大 100。")
...
def _prepare_tool_input(tool_name: str, tool_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if tool_name == "get_user_orders":
        return _compact_payload(
            ...
            start_time=prepared.get("order_start_time"),
            end_time=prepared.get("order_end_time"),
            page=prepared.get("page"),
            page_size=prepared.get("page_size"),
        )
    if tool_name == "get_user_score":
        return _compact_payload(
            start_time=prepared.get("score_start_time"),
            end_time=prepared.get("score_end_time"),
            page=prepared.get("page"),
            page_size=prepared.get("page_size"),
        )
    if tool_name == "get_tickets":
        return _compact_payload(
            ...
            start_time=prepared.get("ticket_start_time"),
            end_time=prepared.get("ticket_end_time"),
            page=prepared.get("page"),
            page_size=prepared.get("page_size"),
        )
