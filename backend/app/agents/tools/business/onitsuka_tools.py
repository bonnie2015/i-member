...
from app.agents.tools.business.onitsuka_client import get_product_detail, list_products, search_products
from app.agents.tools.business.onitsuka_semantics import (
    BRAND_QUERY_ALIASES,
    GENDER_FILTER_IDS,
...
class SearchByIntentInput(BaseModel):
    gender: GenderFilterValue = Field(
        ...
        description=(
            ...
            "说明：女性/男性两个选项多用于偏时尚潮流都市类，女子/男子选项偏休闲经典运动类。"
...
async def onitsuka_get_product_detail(product_id: int, color_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    if "error" in result:
        return result
    return adapt_product_detail(result.get("data") or {})
