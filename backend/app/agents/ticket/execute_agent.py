...
from app.tools import ask_user_tool, finish_step_tool, get_business_tools, get_product_detail_tool
...
def _tool_registry() -> Dict[str, Any]:
    return {
        str(tool.name): tool
        for tool in [*get_business_tools(), get_product_detail_tool(), *get_memory_tools()]
    }
