...
async def guard_node(state: AgentState) -> Dict[str, Any]:
    ...
    result = await ticket_guard_agent.run(
        AgentInput(
    ...
    if result.status != AgentStatus.SUCCESS:
        logger.error(
            "[guard_node] thread_id=%s agent_failed status=%s", thread_id, result.status
        )
    ...
    if decision == "clarify":
        reply = str(
            data.get("reply") or result.reply or "请补充更具体的服务信息。"
        ).strip()
    ...
    if decision == "end_service":
        ...
        logger.info(
            "[guard_node] thread_id=%s decision=end reason=%s", thread_id, final_reason
        )
        return {
            "guard_decision": "end_service",
            "final_reply": reply,
    ...
    if decision != "select_service":
        logger.error(
            "[guard_node] thread_id=%s invalid_decision=%s", thread_id, decision
        )
        return {
            "guard_decision": "end_service",
            ...
            "messages": [
                *state["messages"],
                AIMessage(content="当前服务暂时较忙，请稍等片刻后再试。"),
            ],
        }
    ...
    logger.info(
        "[guard_node] thread_id=%s decision=select service_key=%s",
        thread_id,
        service_key,
    )
    return {
